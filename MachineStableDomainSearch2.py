...
def svmPQV(vecteurs_points_stables, vecteurs_points_instables, vecteurs_points_inconnus):	

	# X est la variable des points d'entrée à analyser
	X_training = np.vstack((vecteurs_points_stables, vecteurs_points_instables,vecteurs_points_inconnus))

	# Y est la variable de prediction/entrainement
	y_training = np.zeros(X_training.shape[0])
	y_training[:vecteurs_points_stables.shape[0]] = 1

	# On applique le svm
	svc = SVC(kernel='linear')
	svc.fit(X_training,y_training)

	# on entraine le svm avec les prediction
	pred_X_0 = svc.predict(X_training[vecteurs_points_stables.shape[0]:])
	pred_X_1 = svc.predict(X_training[:vecteurs_points_stables.shape[0]])

	# On determine le nombre d'erreur du svm
	n_error_classifier = pred_X_1[pred_X_1 == 0].size + pred_X_0[pred_X_0 == 1].size

	# On determine l'equation de l'hyperplan separateur
	
	a = -svc.coef_[0][0]/svc.coef_[0][2]
	b = -svc.coef_[0][1]/svc.coef_[0][2]
	c = -svc.intercept_[0]/svc.coef_[0][2]
	z = lambda x,y: (a*x+b*y+c)
	print "L equation de l hyperplan est U = ", a, ".P + " , b , ".Q + ", c

	X_stable = np.vstack((X_training[:vecteurs_points_stables.shape[0]][pred_X_1 == 1], X_training[vecteurs_points_stables.shape[0]:][pred_X_0 == 1]))
	
	return a, b, c, X_stable, n_error_classifier
...
def Fgroup(X, seqPath, dtaPath, echPath, P_boundaries, Q_boundaries, U_boundaries, U_nominal, P_nominal, precision):
	...
	#etat   = "STEADY STATE UP TO  0.10000D-02 SATISFIED FOR MACHINE EQUATIONS"
	etat2  = "ETAT D'EQUILIBRE A  0.10000D-02 NON VERIFIE DANS LES EQUATIONS SUIVANTES"
	#etat2  = "STEADY STATE UP TO  0.10000D-02 UNSATISFIED IN FOLLOWING EQUATIONS"
...
'''def Exportsvm(liste_groupe_problematique, points_stable, points_instable, points_unknown)

	import xlrd
 	from xlwt import Workbook, Formula
 
	with open(os.path.join(filename + ".xls"),'w') as g:

	for j in range(0,liste_groupe_problematique.shape[0]) 
...
def main(version, visualisation):
	...
	if (version == 1):
		...
		points_MC_stable, points_MC_instable, points_MC_unknown = FMonteCarlo(1500,seqFile, dtaFile, echFile, U_nominal, P_nominal,P_boundaries,Q_boundaries,U_boundaries,precision, MachineName)
		...
		print "Points stables   : ", points_MC_stable_si.shape[0]
		print "Points instables : ", points_MC_instable_si.shape[0]
		print "Points inconnus  : ", points_MC_unknown_si.shape[0]
		print "Points stables ok: ", points_MC_stable_OK_si.shape[0]
		print "Points stables   : ", points_MC_stable_si
		print "Points instables : ", points_MC_instable_si
		print "Points inconnus  : ", points_MC_unknown_si
		print "Points stables ok: ", points_MC_stable_OK_si
		
		# on supprime les points pour lesquels P<0
		points_stables_fin = np.zeros((0,3))
		for i in range(0,points_MC_stable_OK_si.shape[0]):
			if (points_MC_stable_OK_si[i,0]>0):
				points_stables_fin = np.vstack((points_stables_fin,points_MC_stable_OK_si[i,:]))
		
		points_instables_fin = np.zeros((0,3))
		for i in range(0,points_MC_instable_si.shape[0]):
			if (points_MC_instable_si[i,0]>0):
				points_instables_fin = np.vstack((points_instables_fin,points_MC_instable_si[i,:]))
		
		points_unknown_fin = np.zeros((0,3))
		for i in range(0,points_MC_unknown_si.shape[0]):
			if (points_MC_unknown_si[i,0]>0):
				points_unknown_fin = np.vstack((points_unknown_fin,points_MC_unknown_si[i,:]))
		
		
		# on garde le minimum de points instables à l'aide d'un classifier SVM
		a, b, c, X_stable, n_error_classifier = svmPQV(points_stables_fin, points_instables_fin, points_unknown_fin)
		print "L equation de l'hyperplan séparateur est V = a*P+b*Q+c avec a= ", a, " b= ", b, "et c= ",c
	
		
		# Export domain
		CreateMachineDomain.FexportCSV(X_stable,"ampl_generators_domains_MCsvmPQU",MachineRefName,MachineName,U_nominal,O)
		
		hull = ConvexHull(X_stable)
		
		# Visualisation du MC après SVM
		if (visualisation == 1 or visualisation == 0):
			
								
			'''tmp = np.linspace(-5,5,X_fin.shape[0])
			x,y = np.meshgrid(tmp,tmp)
			z = lambda x,y: (a*x+b*y+c)
			ax.plot_surface(x, y, z(x,y))
			
			# ax.plot3D(X_fin[:,0], X_fin[:,1], X_fin[:,2],'ob', 'o', color = 'blue')
			# ax.plot3D(X_training[y_training==1,0], X_training[y_training==1,1], X_training[y_training==1,2],'sr')
			ax.plot3D(points_MC_instable_si[:,0],points_MC_instable_si[:,1],points_MC_instable_si[:,2], 'o', color="red")
			ax.plot3D(points_MC_unknown_si[:,0],points_MC_unknown_si[:,1],points_MC_unknown_si[:,2], 'o', color="yellow")
			ax.plot3D(X_fin[:,0], X_fin[:,1], X_fin[:,2],'o', color = "blue")
			# ax.plot3D(points_MC_stable_OK_si[:,0],points_MC_stable_OK_si[:,1],points_MC_stable_OK_si[:,2], 'o', color='green')
			# ax.plot3D(X_fin[:,0], X_fin[:,1], X_fin[:,2],'ob', color = 'blue')'''
			
			plt.plot(points_instables_fin[:,0],points_instables_fin[:,1],points_instables_fin[:,2], 'ob', color="red")
			plt.plot(points_unknown_fin[:,0],points_unknown_fin[:,1],points_unknown_fin[:,2], 'ob', color="yellow")
			plt.plot(points_stables_fin[:,0],points_stables_fin[:,1],points_stables_fin[:,2], 'ob', color='green')
			# plt.plot(X_stable[:,0],X_stable[:,1],X_stable[:,2], 'sr', color='blue')
			
			for sommets in hull.simplices:
				plt.plot(X_stable[sommets,0], X_stable[sommets,1], X_stable[sommets,2], 'k-')	
				
			ax.set_xlabel('P', fontsize='xx-large')
			ax.set_ylabel('Q', fontsize='xx-large')
			ax.set_zlabel('U', fontsize='xx-large')
			fig.suptitle(MachineName, fontsize='xx-large')
			fig.savefig(MachineName + '_MCsvm.png')
			plt.draw()
